...
public class InvestmentService {
    ...
    private static final org.slf4j.Logger log = org.slf4j.LoggerFactory.getLogger(InvestmentService.class);
    ...
    public InvestmentResponseDTO invest(InvestmentRequestDTO req) {
        ...
        // ── Resolve entities ──────────────────────────────────────────
        User user = userRepository.findById(req.getUserId()).orElseThrow(() -> new ResourceNotFoundException("User", "id", req.getUserId()));
        MutualFund fund = fundRepository.findById(req.getFundId()).orElseThrow(() -> new ResourceNotFoundException("MutualFund", "id", req.getFundId()));
        if (!fund.isActive()) {
        ...
        // ── Build & save ──────────────────────────────────────────────
        Investment investment = Investment.builder().user(user).mutualFund(fund).amount(req.getAmount()).date(LocalDate.now()).units(units).purchaseNav(nav).type(type).status(Investment.InvestmentStatus.ACTIVE).build();
        investment = investmentRepository.save(investment);
        log.info("Investment created: id={}, userId={}, fundId={}, amount={}", investment.getId(), user.getId(), fund.getId(), req.getAmount());
    ...
    public List<InvestmentResponseDTO> getUserInvestments(Long userId) {
        ...
        return investmentRepository.findByUser_Id(userId).stream().map(this::toDTO).toList();
    }
    ...
    public List<InvestmentResponseDTO> getActiveUserInvestments(Long userId) {
        validateUserExists(userId);
        return investmentRepository.findByUser_IdAndStatus(userId, Investment.InvestmentStatus.ACTIVE).stream().map(this::toDTO).toList();
    }
    ...
    public List<InvestmentResponseDTO> getUserInvestmentsByFund(Long userId, Long fundId) {
        return investmentRepository.findByUser_IdAndMutualFund_Id(userId, fundId).stream().map(this::toDTO).toList();
    }
    ...
    public TotalInvestmentDTO calculateTotalInvestment(Long userId) {
        ...
        List<Investment> active = investmentRepository.findByUser_IdAndStatus(userId, Investment.InvestmentStatus.ACTIVE);
        BigDecimal portfolioValue = active.stream().map(i -> i.getUnits().multiply(i.getMutualFund().getNav())).reduce(BigDecimal.ZERO, BigDecimal::add).setScale(2, RoundingMode.HALF_UP);
        // ── Gain / loss ────────────────────────────────────────────────
        ...
        return TotalInvestmentDTO.builder().userId(userId).totalActiveAmount(totalActive).grandTotalAmount(grandTotal).currentPortfolioValue(portfolioValue).overallGainLoss(gainLoss).overallGainLossPercent(gainLossPercent).distinctFundsCount(fundsCount != null ? fundsCount : 0L).totalTransactions(txCount != null ? txCount : 0L).build();
    }
    ...
    public InvestmentResponseDTO redeem(Long investmentId) {
        Investment inv = investmentRepository.findById(investmentId).orElseThrow(() -> new ResourceNotFoundException("Investment", "id", investmentId));
    ...
    private InvestmentResponseDTO toDTO(Investment inv) {
        ...
        return InvestmentResponseDTO.builder().id(inv.getId()).userId(inv.getUser().getId()).userName(inv.getUser().getName()).fundId(inv.getMutualFund().getId()).fundName(inv.getMutualFund().getFundName()).amount(inv.getAmount()).units(inv.getUnits()).purchaseNav(inv.getPurchaseNav()).date(inv.getDate()).type(inv.getType().name()).status(inv.getStatus().name()).currentValue(currentValue).gainLoss(gainLoss).gainLossPercent(gainLossPct).build();
    }

    public InvestmentService(final InvestmentRepository investmentRepository, final UserRepository userRepository, final MutualFundRepository fundRepository) {

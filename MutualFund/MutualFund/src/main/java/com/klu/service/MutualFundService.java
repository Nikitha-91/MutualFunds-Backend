...
public class MutualFundService {
    ...
    public MutualFundResponseDTO addFund(MutualFundRequestDTO req) {
        ...
        MutualFund fund = MutualFund.builder().fundName(req.getFundName().trim()).category(req.getCategory().toUpperCase().trim()).riskLevel(resolveRiskLevel(req.getRiskLevel())).returns(req.getReturns()).nav(req.getNav()).description(req.getDescription()).active(true).build();
    ...
    public List<MutualFundResponseDTO> getAllFunds() {
        return fundRepository.findByActiveTrue().stream().map(this::toDTO).toList();
    }
    ...
    public List<MutualFundResponseDTO> searchByName(String fundName) {
        return fundRepository.findByFundNameContainingIgnoreCase(fundName).stream().filter(MutualFund::isActive).map(this::toDTO).toList();
    }
    ...
    public List<MutualFundResponseDTO> getByCategory(String category) {
        return fundRepository.findByCategoryIgnoreCaseAndActiveTrue(category).stream().map(this::toDTO).toList();
    }
    ...
    public List<MutualFundResponseDTO> getByRiskLevel(String riskLevel) {
        return fundRepository.findByRiskLevelAndActiveTrue(resolveRiskLevel(riskLevel)).stream().map(this::toDTO).toList();
    }
    ...
    public List<MutualFundResponseDTO> getByMinReturns(BigDecimal minReturns) {
        return fundRepository.findByMinReturns(minReturns).stream().map(this::toDTO).toList();
    }
    ...
    private MutualFund findOrThrow(Long id) {
        return fundRepository.findById(id).orElseThrow(() -> new ResourceNotFoundException("MutualFund", "id", id));
    }
    ...
    private MutualFund.RiskLevel resolveRiskLevel(String riskLevel) {
        try {
            ...
            throw new BadRequestException("Invalid riskLevel \'" + riskLevel + "\'. Accepted: LOW | MODERATE | HIGH | VERY_HIGH");
        }
    }

    private void validateRequest(MutualFundRequestDTO req) {
        if (req.getFundName() == null || req.getFundName().isBlank()) throw new BadRequestException("fundName is required.");
        if (req.getCategory() == null || req.getCategory().isBlank()) throw new BadRequestException("category is required.");
        if (req.getRiskLevel() == null || req.getRiskLevel().isBlank()) throw new BadRequestException("riskLevel is required.");
        if (req.getReturns() == null) throw new BadRequestException("returns is required.");
        if (req.getNav() == null) throw new BadRequestException("nav is required.");
    }
    ...
    private MutualFundResponseDTO toDTO(MutualFund fund) {
        return MutualFundResponseDTO.builder().id(fund.getId()).fundName(fund.getFundName()).category(fund.getCategory()).riskLevel(fund.getRiskLevel().name()).returns(fund.getReturns()).nav(fund.getNav()).description(fund.getDescription()).active(fund.isActive()).build();
    }

    public MutualFundService(final MutualFundRepository fundRepository) {

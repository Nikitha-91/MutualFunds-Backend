...
public class RecommendationService {
    ...
    public List<RecommendationDTO> getTopBuyOpportunities(Long advisorId, int limit) {
        ...
        return fundRepository.findByActiveTrue().stream().sorted(Comparator.comparingDouble(f -> -f.getReturns().doubleValue())).limit(limit).map(f -> toDTO(f, advisor, "BUY", "High annualised return fund suitable for growth-oriented investors.")).collect(Collectors.toList());
    }
    ...
    public List<RecommendationDTO> getRecommendationsByRisk(Long advisorId, String riskLevel) {
        ...
        try {
            ...
            throw new BadRequestException("Invalid risk level \'" + riskLevel + "\'. Use LOW | MODERATE | HIGH | VERY_HIGH");
        }
        return fundRepository.findByRiskLevelAndActiveTrue(level).stream().sorted(Comparator.comparingDouble(f -> -f.getReturns().doubleValue())).map(f -> toDTO(f, advisor, inferAction(level), "Fund recommended based on " + level.name() + " risk profile.")).collect(Collectors.toList());
    }
    ...
    public List<RecommendationDTO> getRecommendationsByCategory(Long advisorId, String category) {
        User advisor = resolveAdvisor(advisorId);
        return fundRepository.findByCategoryIgnoreCaseAndActiveTrue(category).stream().sorted(Comparator.comparingDouble(f -> -f.getReturns().doubleValue())).map(f -> toDTO(f, advisor, "BUY", "Strong " + category.toUpperCase() + " category fund.")).collect(Collectors.toList());
    }
     ...
     */
    @Transactional(readOnly = true)
    public RecommendationDTO createRecommendation(Long advisorId, Long fundId, String action, String rationale) {
        User advisor = resolveAdvisor(advisorId);
        MutualFund fund = fundRepository.findById(fundId).orElseThrow(() -> new ResourceNotFoundException("MutualFund", "id", fundId));
        if (!fund.isActive()) {
            throw new BadRequestException("Fund \'" + fund.getFundName() + "\' is inactive — cannot recommend it.");
        }
        ...
        return toDTO(fund, advisor, normalizedAction, rationale != null && !rationale.isBlank() ? rationale : "Advisor recommendation.");
    }
    ...
    private User resolveAdvisor(Long advisorId) {
        ...
        return userRepository.findById(advisorId).orElseThrow(() -> new ResourceNotFoundException("User", "id", advisorId));
    }
     ...
     * Entity → DTO mapper.
     */
    private RecommendationDTO toDTO(MutualFund fund, User advisor, String action, String rationale) {
        return RecommendationDTO.builder().fundId(fund.getId()).fundName(fund.getFundName()).advisorId(advisor.getId()).advisorName(advisor.getName()).rationale(rationale).action(action).riskLevel(fund.getRiskLevel().name()).category(fund.getCategory()).returns(fund.getReturns().doubleValue()).date(LocalDate.now()).build();
    }

    public RecommendationService(final MutualFundRepository fundRepository, final UserRepository userRepository) {
